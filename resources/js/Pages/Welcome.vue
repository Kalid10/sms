...
function login(simulation = false, userType = "admin") {
    ...
    router.post(
        "/login",
        {
            emailOrPhone: simulation ? "biniyam@rigel.studio" : email,
            password: "password",
        },
        {
            onSuccess: () => {
...
function addSchoolSchedule(event, i = 0) {
    ...
    router.post("/school-schedules/create", dates[i], {
        onSuccess: () => {
            if (i < dates.length - 1) addSchoolSchedule(event, i + 1);
        },
        onError: (error) => {
            console.log("Error");
            console.log(error);
