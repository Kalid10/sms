    ...
    <div>
        <div class="flex w-full flex-col space-x-2 md:flex-row">
            ...
            <Card
                ...
                subtitle="Add or remove roles to update the user's abilities"
            >
                <template #icon>
                    <IdentificationIcon />
                </template>
                ...
                <ul class="max-h-80 overflow-auto">
                    <li
                        v-for="(role, r) in newUserRoles"
                        :key="r"
                        class="group flex items-center gap-2 rounded-md p-3 odd:bg-neutral-100"
                    >
                        <InformationCircleIcon
                            class="h-3 w-3"
                        ></InformationCircleIcon>
                        <span class="grow font-mono text-sm">{{
                            role.name
                        }}</span>
                        <TrashIcon
                            class="h-4 w-4 stroke-gray-500 stroke-0 group-hover:stroke-2"
            ...
            <div class="max-w-md md:p-2">
                <div
                    class="mx-auto my-10 max-w-2xl rounded-xl bg-white p-8 shadow shadow-slate-300"
                >
                    <div
                        class="flex flex-row items-center justify-between space-x-reverse"
                    >
        ...
        <div class="rounded-xl bg-white p-8 md:pt-14">
            ...
            <ul
                class="flex max-w-2xl flex-col divide-y divide-gray-200 dark:divide-gray-700"
            >
                <li
                    v-for="activityLog in activities"
                    :key="activityLog.date"
                    class="justify-center pb-3 sm:pb-4"
                >
                    <div class="flex items-center space-x-5 p-1">
                        ...
                        <div class="min-w-0 flex-1">
                            <p
                                class="text-md truncate font-medium text-brand-text-500 dark:text-white"
                            >
                            ...
                            <div
                                class="truncate text-sm text-brand-text-300 dark:text-brand-text-250"
                            >
                                Role <span> {{ activityLog.action }}</span> by
                                <span class="">{{ activityLog.givenby }}</span>
                            </div>
                        </div>
                        <div
                            class="inline-flex items-center text-base font-normal text-brand-text-250 dark:text-white"
                        >
                            {{ activityLog.date }}
                        </div>
                    </div>
...
const availableRoles = computed(() => {
    return roles.value.filter(
        (a_role) =>
            !newUserRoles.value.map((role) => role.name).includes(a_role.name)
    );
});
