<template>
    ...
    <form class="flex min-h-max w-full flex-col items-center justify-center py-4">
        ...
        <Heading class="mb-1 text-center !font-normal text-black" size="md">Welcome to the</Heading>
        ...
        <div class="relative flex items-center">
            ...
            <div class="absolute top-0 right-0 -mr-4 flex translate-x-full flex-col gap-2">
                <!--                <PencilIcon-->
        ...
        <div class="mt-8 flex">
            <h3 class="max-w-md text-center text-sm text-gray-500">
                <span><InformationCircleIcon class="mb-1 inline h-4 w-4 stroke-2"/></span>
                Please enter the <span class="text-black">start date</span> of the new school year and
                the <span class="whitespace-nowrap text-black">number of semesters</span> it includes.
                You can also change the school year name by clicking the <span><PencilIcon
                class="inline h-3 w-3 stroke-black stroke-2"/></span> icon on the right.
                <span class="inline">
        ...
        <div class="mt-8 flex flex-col gap-3">
            <DatePicker
                v-model="formData.start_date" :label="!! formData.start_date ? 'Choose a Start Date' : ''"
                :label-location="!! formData.start_date ? 'inside' : ''"
                :minimum="new Date()" class="w-72"
                placeholder="Choose a Start Date"
            />
            <SelectInput
                v-model="formData.number_of_semesters"
                :label="!! formData.number_of_semesters ? 'Number of Semesters' : ''"
                :label-location="!! formData.number_of_semesters ? 'inside' : ''"
                :options="noOfSemesters"
                class="w-72"
                placeholder="Number of Semesters"
            />
            <PrimaryButton :disabled="! formComplete" class="w-72" @click="handleSubmit">Create and Proceed
            </PrimaryButton>
        </div>
...
import {InformationCircleIcon, PencilIcon, ArrowPathIcon} from "@heroicons/vue/24/outline";
