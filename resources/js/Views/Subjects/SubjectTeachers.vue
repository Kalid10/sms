        ...
        <template #filter>
            ...
            <div class="flex items-end justify-between gap-3">
                <div class="flex flex-col">
                    <h3 class="whitespace-nowrap font-semibold">{{ subject['full_name'] }}</h3>

                    <h3 class="whitespace-nowrap text-sm text-gray-500">List of teachers in the {{
                            subject['short_name']
                        }} Faculty</h3>
                </div>
                <div class="flex grow flex-col justify-end gap-2 md:flex-row">
                    <TextInput v-model="searchKey" class="grow" placeholder="Search for a teacher by name"/>

                    <RadioGroup
v-model="filteredLevelCategory" :options="levelCategoryOptions"
            ...
            <div class="flex flex-col items-center justify-center">
                ...
                <div v-if="searchKey.length">
                    ...
                    <p v-else class="text-center text-sm text-gray-500">
                        Your search query "<span class="font-medium text-black">{{ searchKey }}</span>" did not match
...
:class="genderLabels[data['gender']]"
                              class="scale-[.7] rounded-xl px-2.5 py-0.5 font-semibold">{{ data['gender'] }}</span>
                    </div>
        ...
        <template #batches-column="{data}">
            ...
            <div class="flex items-center justify-end gap-3 py-2">
                <Link
v-for="(batch, b) in data" :key="b"
                      :class="levelCategoryLabels[levelCategories.indexOf(batch['level_category'])]"
