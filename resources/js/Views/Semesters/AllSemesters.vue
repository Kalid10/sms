    ...
    <div class="container mx-auto flex w-full flex-col items-center gap-3">
        ...
        <div class="flex w-full flex-col gap-3 xl:flex-row">
            ...
            <div class="flex w-full flex-col gap-3 sm:flex-row xl:w-fit">
                ...
                <SelectInput
                    v-model="selectedYear" class="w-full sm:w-1/3 sm:max-w-xs xl:w-44" :options="years"
...
const semesters = computed(() => {
    return semester.semesters.filter((semester) => {
        return semester.status.includes(selectedStatus.value) &&
            new Date(semester.school_year.start_date).getFullYear().toString().includes(selectedYearLabel.value === 'All years' ? '' : selectedYearLabel.value) &&
