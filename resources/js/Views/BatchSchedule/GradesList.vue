<template>
    <div
        class="flex h-full w-56 flex-col overflow-auto rounded-lg bg-brand-100 py-6 pl-2"
    >
        ...
        <ul class="flex flex-col overflow-auto">
            <li v-for="(level, l) in levels" :key="l">
                <!--                todo:fix Cannot read properties of undefined (reading 'id')-->
                <Link
                    :class="{
                        'rounded-l-md bg-brand-550 text-white':
                            level.id === activeLevelId,
                    }"
                    class="flex items-center justify-between py-3 pl-4"
                    :href="`?batch_id=${level['batches'][0]['id']}`"
                >
                    <Heading
                        :class="[
                            !(level.id === activeLevelId)
